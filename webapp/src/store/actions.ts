...
const actions = wrapActions({
  ...
  async donateAnonymously({state}, { amount, name, phone, email}: {amount: number; name: string; phone: string; email: string }) {
    if (!state.user) {
      await Donations.initiateAnonymousDonation({ amount, name, phone, email });
    }
  },

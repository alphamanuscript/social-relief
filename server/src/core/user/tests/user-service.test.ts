...
describe('UserService tests', () => {
  ...
  describe('nominateBeneficiary', () => {
    describe('when nominator is a middleman', () => {
      test('should add represented donors to existing beneficiary', async () => {
        const res = await createDefaultService().nominateBeneficiary({ phone: '254700444444', email: null, nominator: 'middleman1' });
        expect(res).toEqual({ _id: 'beneficiary1', phone: '254700444444', email: null });
      ...
      test('should also add middleman to beneficiary donors if middleman is also a donor', async () => {
        const now = new Date();
        const res = await createDefaultService().nominateBeneficiary({ phone: '254700444444', email: null, nominator: 'donorMiddleman1' });
      ...
      test('should create beneficiary if does not already exist', async () => {
        const now = new Date();
        const res = await createDefaultService().nominateBeneficiary({ phone: '254711222333', email: null, nominator: 'donorMiddleman1' });
  ...
  describe('nominateMiddleman', () => {
    test('should add donor to the list of donors the middleman represents and add middleman role', async () => {
      const now = new Date();
      const res = await createDefaultService().nominateMiddleman({ phone: '254700222222', email: null, nominator: 'donor1' });
      expect(res).toEqual({ _id: 'donor2', phone: '254700222222', email: null });
    ...
    test('should create new user if does not exist', async () => {
      const now = new Date();
      const res = await createDefaultService().nominateMiddleman({ phone: '254766111222', email: null, nominator: 'donor1' });
    ...
    test('should allow nominating user who is already a middleman for other donors', async () => {
      const now = new Date();
      const res = await createDefaultService().nominateMiddleman({ phone: '254700555555', email: null, nominator: 'donor2' });
    ...
    test('allows nominating beneficiary as middleman', async () => {
      const now = new Date();
      const res = await createDefaultService().nominateMiddleman({ phone: '254700444444', email: null,  nominator: 'donor1' });
      expect(res._id).toBe('beneficiary1');
      const updatedMiddleman = await usersColl().findOne({ _id: res._id });
  ...
  describe('getAllMiddlemenByUser', () => {
    test('should return all the middleman for the specified user', async () => {
      ...
      expect(res).toEqual([{ _id: 'donorMiddleman1', phone: '254700555555', email: null }, { _id: 'middleman1', phone: '254700333333', email: null }]);
    });
  });
});
